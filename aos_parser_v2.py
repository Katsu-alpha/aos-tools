...
class AOSParser:
    ...
    def __init__(self, files, cmds=(AP_DATABASE_LONG_TABLE, AP_ACTIVE_TABLE), activeonly=True, merge=False, encoding='utf-8', callback=None):
        """
        tech-support または show コマンドログファイルをパースし、cmds 配列にあるコマンドの
        出力を2 次元配列に格納
            ...
            if not in_cmd:
                for cmd in cmds:
                    ...
                    if re.search(pat, line):
                        ...
                        self.num = 0
                        self.origdata = [line]
                        self.startlno = self.lno
                        continue

                continue

        # in_cmd = true
            self.origdata.append(line)
