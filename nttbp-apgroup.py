...
for fn in args.files:
    ...
    for r in ap_db_tbl:
        ...
        if apg not in apg_controllers:
            apg_controllers[apg] = set()
        apg_controllers[apg].add(controller)
        ...
        for vap in vap_set:
            if apg not in apg_vaps:
                apg_vaps[apg] = set()
                vap_count[apg] = {}
            apg_vaps[apg].add(vap)
            if vap not in vap_count[apg]:
                vap_count[apg][vap] = 0
            vap_count[apg][vap] += 1
...
print("\nCreating table...")
results = []
for apg in sorted(apg_apcount.keys()):
    if apg not in apg_vaps:
